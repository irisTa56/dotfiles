...
# Sourced from ~/.zshenv, which every zsh reads, a tool's non-interactive one
# included, so what an agent's shell needs goes here.
...
# Here rather than .zshrc, so a tool's shell handles characters the same way
# an interactive one does; Claude Code's snapshot does not carry it over.
...
# the window npm gets from its user config (set by setup_dotfiles.sh). uv
# gets it only where no project lock is written: a user-wide one is baked
# into each project's uv.lock and then conflicts with anyone locking without
# it (astral-sh/uv#18775), so a project sets its own in pyproject.toml.
