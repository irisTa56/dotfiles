...
- Checks: `mise run pre-commit` from this repository's root.
  ...
  - It also leaves out skill evals, which spend plan usage: when a skill with an `evals/` directory changes, run `claude plugin eval .claude/skills/<name> --trust-plugin --no-publish --judge-model sonnet`, and where a grader fails in any of a case's runs, rerun that case (`--case <case>`) on the skill as it stood before the edit and compare the grader's pass rate across runs, since graders judged by a model flip between runs.
