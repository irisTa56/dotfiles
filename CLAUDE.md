...
- Checks: `mise run pre-commit` from this repository's root.
  ...
  - It also leaves out skill evals, which spend plan usage: when an edit to a skill with an `evals/` directory rewrites or removes a rule its graders check, or rewrites the skill at large, run `claude plugin eval .claude/skills/<name> --trust-plugin --no-publish --judge-model sonnet`.
