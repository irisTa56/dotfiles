...
It then runs `apm update` on that skill, which moves the pin to the pushed commit; until that `apm.lock.yaml` change reaches `main`, an `apm install` from `main` restores the old copy.
