---
description: A finding on prose that holds only when one sentence is read apart from the next
max_turns: 8
allowed_tools: [Skill]
---
...
The finding: "'Run `make migrate` before deploying' is unconditional. A reader will run migrations on every release. Rewrite the first sentence as 'If the release has schema changes, run `make migrate` before deploying.'"
