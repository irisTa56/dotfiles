...
- The Bash tool's shell is zsh, which [does not split an unquoted `$var` into words](https://zsh.sourceforge.io/FAQ/zshfaq03.html#l19) as `sh` and `bash` do. A command built from a variable holding several arguments receives them as one, and usually fails outright rather than doing part of the job.
