...
- Apply an edit that is not mechanical with the edit tool rather than through a script; the tool puts the change where the user is reading, and a script puts only the instructions that would produce it.
- Commit each step — a review round, or the work of one instruction — so the step has a diff of its own and a state to return to.
...
- When they ask to see the diff, or to comment on it, open it in `crit` against that baseline rather than rendering one yourself.
