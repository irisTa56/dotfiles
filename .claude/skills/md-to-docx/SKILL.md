...
The script depends on two npm packages installed globally:

```bash
npm install -g docx marked
```

Check / install before running. The script requires them via `NODE_PATH=$(npm root -g)`.

## Usage

```bash
NODE_PATH=$(npm root -g) node ~/.claude/skills/md-to-docx/scripts/build_docx.js <input.md> <output.docx> [options]
```
...
| Math | TeX → Unicode fallback (italic): `\frac{a}{b}` → `(a) ÷ (b)`, `\geq` → `≥`, etc. |
...
- **Math**: rendered as Unicode text, not native Word equations (OMML).
...
- **Math fallback**: a small TeX → Unicode converter handles `\frac`, `\text`, `\geq`, `\leq`, `\gg`, `\times`, `\cdot`, subscripts, etc. Display math becomes a centered italic paragraph; inline math becomes an italic run.
...
- **`Cannot find module 'docx'`** — run `npm install -g docx marked`. The script reads from `NODE_PATH=$(npm root -g)`.
...
- **Math is not rendered as a Word equation** — intentional. Native OMML generation from arbitrary TeX is out of scope; the Unicode fallback is readable and consistent. If full OMML is required, use pandoc with `--from=markdown+tex_math_dollars` and accept its limitations.
