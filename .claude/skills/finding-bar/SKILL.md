...
- For a value the change takes in rather than produces, it is what whoever supplies that value can make the code branch on, or can make a reader of that value do.
  - An agent that executes the prose the value reaches is one such reader, and takes the value as instruction.
