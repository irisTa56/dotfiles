...
- When a command fails for want of a key, look for it there rather than in `.env`. Never write a key into `.env` or mise's `[env]`; a key not yet stored is the user's to add, with `fnox set <NAME> --provider keychain`.
