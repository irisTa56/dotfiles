...
- **Lexical compound → no space.** A *single* English token can fuse with adjacent Japanese into one word; join it, since a space would break the word.
  - Fuses when the Japanese side is a bound affix or positional suffix (`製` / `系` / `版`, `上` / `内` / `中` / `間` …), which join even though some read as English prepositions.
  - Fuses when the pair reads back in English as a self-contained noun phrase, the Japanese element being a content word: `Docker環境` = "Docker environment", `Rust言語` = "Rust language", `生成AI` = "generative AI".
  - Does *not* fuse when the Japanese element reads in English as a preposition or relational word rather than a content noun, and is not one of the bound suffixes above (`gRPC経由` = "via gRPC") — rephrase it; see the next case.
  - OK: `生成AI`, `Docker環境`, `API利用`, `Kubernetes上`, `Rust製`, `Elixir系`.
  - NG: `生成 AI`, `Docker 環境`, `API 利用`, `Kubernetes 上`, `Rust 製`.
- **Not a compound → avoid the bare space; rephrase.** When the pair is not a noun compound — either because the Japanese element reads as a preposition or relational word and is not one of the bound suffixes above (`gRPC 経由の…` = "via gRPC"), or because the English side is a multi-word term (`Open Source 開発`) — do not leave the English and Japanese sitting side by side with only a space. Rephrase by one of:
