...
- Write the intent, not a trace of the mechanism. Do not restate what the reader derives from the code or the config, such as a name said again as prose. What survives is why it is this way, a fact reading would not yield, or a derivation that rules out a wrong move.
...
- A flat list of short one-sentence bullets can still be wrong, so clause count is not the check. Ask of every list what nesting or splitting would say about it.
