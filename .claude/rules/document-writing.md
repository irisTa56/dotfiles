...
- When citing a place in a file the same work is still editing, anchor on the quoted text rather than on a line number no commit pins; the edit that writes the citation is usually the edit that moves the line.
