...
mise [loads every executable under a task directory](https://mise.jdx.dev/tasks/file-tasks.html), at whatever depth, so a file placed here to be run by another task would become a task of its own in every consuming repository, under a name nobody chose.
